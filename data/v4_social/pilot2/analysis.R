...
df.demo = read.csv('demo.csv', stringsAsFactors = F) %>% 
  filter(subject != '5f70d47e79bdca6d8099ecb2') %>% # glitched
  arrange(subject) %>%
  rowwise() %>%
...
df.s2 = read.csv('s2.csv', stringsAsFactors = F) %>% filter(subject %in% subjlist) %>%
  ...
  mutate(subject = factor(subject), subject.num = as.numeric(subject))
         ...
         #most_preferred = least_preferred_temp) %>% # this is needed in old direction q's b/c columns were reversed by accident
...
test = df.s2.filt %>%
  filter(type == 'direction' & attribute == 'Indoors vs. outdoors') %>%
  select(subject, rating, least_preferred, most_preferred)
...
ggplot(df.demo.filt, aes(x = mindfulness, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
m.mindfulness = lm(accuracy.best.best ~ mindfulness + chosen.model.fac + avg.model.ll, data = df.demo.filt)
...
ggplot(df.demo.filt, aes(x = icar_num_correct, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')
summary(lm(accuracy.best.best ~ icar_num_correct + chosen.model.fac + avg.model.ll, df.demo.filt))
summary(lm(chosen.model.ll ~ icar_num_correct + chosen.model.fac + avg.model.ll, df.demo.filt))
