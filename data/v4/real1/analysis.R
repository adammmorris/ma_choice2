...
df.demo = read.csv('demo.csv', stringsAsFactors = F) %>% arrange(subject) %>%
  ...
  mutate(chosen.oneatt = factor(lex_real > 50, c(F,T), one.att.levels),
         ...
         norm.binatts = factor(binatts_norm > 50, c(F,T), bin.atts.levels),
...
df.demo = df.demo %>%
  mutate(chosen.model.num = ifelse(chosen.oneatt == 'One',
         ...
         norm.model.num = ifelse(norm.oneatt == 'One',
                                   ifelse(norm.binatts == 'Binary', 6, 5),
                                   ifelse(norm.binwts == 'Binary',
                                          ifelse(norm.binatts == 'Binary', 4, 3),
                                          ifelse(norm.binatts == 'Binary', 2, 1))),
...
df.demo$chance.ll.magnitude = df.cv.best$V4
...
df.attributes.subj = df.attributes %>% group_by(subject) %>%
  summarize(num_same = sum(same & (rating.wad > .1)),
            ...
            accuracy.best.best.rank = cor(fitted.weight.best, rating.best.signed, method = 'kendall'),
            test.wad.ew = cor(fitted.weight.wad, fitted.weight.ew),
            test.wad.wp = cor(fitted.weight.wad, fitted.weight.wp),
            test.wad.tal = cor(fitted.weight.wad, fitted.weight.tal),
            test.ew.wp = cor(fitted.weight.ew, fitted.weight.wp),
            test.ew.tal = cor(fitted.weight.ew, fitted.weight.tal),
            test.wp.tal = cor(fitted.weight.wp, fitted.weight.tal))
...
df.demo.heat.oneatt = df.demo %>% group_by(chosen.oneatt, best.model.oneatt) %>%
  summarize(num.subj = n())
ggplot(df.demo.heat.oneatt, aes(x = best.model.oneatt, y = chosen.oneatt,
                         fill = num.subj)) +
  geom_tile() +
  labs(y = '\nSelf-reported model', x = 'Best-fitting model') +
  #scale_fill_brewer(palette = 'YlOrRd') +
...
df.demo.filt = df.demo.filt %>%
  mutate(oneatt.error = ifelse(oneatt.correct, NA, ifelse(best.model.oneatt == 'One', 1, -1)),
         binwts.error = ifelse(binwts.correct == T | is.na(binwts.correct), NA, ifelse(best.model.binwts == 'Binary', 1, -1)),
         binatts.error = ifelse(binatts.correct, NA, ifelse(best.model.binatts == 'Binary', 1, -1))
  )
mean(df.demo.filt$overestimated)
...
t.test(df.demo.filt$chose.correct.model - 1/6)

get.ci(df.demo.filt$chose.correct.model[df.demo.filt$best.model.ll.magnitude > 0.9])
...
get.ci(df.demo.filt$pct.features.correct)
get.ci(df.demo.filt$num.features.correct)
mean(df.demo.filt$binwts.correct[!is.na(df.demo.filt$binwts.correct)], na.rm = T)

ggplot(df.demo.filt, aes(x = num.features.correct)) +
  geom_histogram()

mean(df.demo.filt$best.model.binatts == 'Binary',na.rm=T)
...
get.ci(df.demo.filt$chosen.model.ll[df.demo.filt$best.model.ll.magnitude > 0.9])
get.ci(rand.lls)
...
get.ci(df.demo.filt$chosen.model.dist[!df.demo.filt$chose.correct.model])
...
df.demo.filt = df.demo.filt %>%
  mutate(chosen.model.kappa = (chosen.model.ll.unnormed - chance.ll.magnitude) / (best.model.ll.magnitude - chance.ll.magnitude))

ggplot(df.demo.filt, aes(chosen.model.kappa)) +
  geom_histogram(color = 'black', bins = 50) +
  labs(x = "",
       y = "") +
  scale_y_continuous(breaks = NULL) +
  scale_x_continuous(breaks = c(-.5,0,.5,1)) +
  #geom_vline(xintercept = mean(df.demo.filt$chosen.model.kappa), size = 1.5, linetype = 'longdash') +
  geom_vline(xintercept = 0, size = 1, linetype = 'dotted', color = 'red') +
  geom_vline(xintercept = 1, size = 1, linetype = 'dotted', color = 'red')
...
## parameter awareness
# across all points
ggplot(df.attributes.filt, aes(x = rating.best.signed, y = fitted.weight.best)) +
  geom_point(color = 'gray') +
  theme_black() +
  geom_smooth(method='lm',color = 'white') +
...
for (i in 1:length(rand.errs)) {
  ...
  for (j in 1:nrow(df.demo.filt)) {
    ...
    # }
    reported = sample(df.attributes$rating.best.signed[subj.rows])
    ...
    test[j] = cor(reported, fitted.best)
  }
...
ggplot(df.demo.filt, aes(x = accuracy.best.best)) +
  ...
  geom_vline(xintercept = mean(df.demo.filt$accuracy.best.best), color = 'gray') +
  labs(x = 'Weight accuracy', y = '# of subjects') +
  #labs(x = '', y = '') +
  scale_y_continuous(breaks = NULL) +
  scale_x_continuous(limits = c(-.2, 1.1), breaks = c(0, .5, 1)) +
  theme_black()
ggplot(df.demo.filt %>% filter(best.model.oneatt == 'Multiple'), aes(x = accuracy.best.best)) +
  geom_histogram(alpha = .3, color = 'white') +
  geom_vline(xintercept = mean(rand.errs), linetype = 'dashed', color = 'gray') +
  geom_vline(xintercept = mean(df.demo.filt$accuracy.best.best), linetype = 1, color = 'gray') +
  labs(x = 'Parameter accuracy', y = '# of subjects') +
  #scale_y_continuous(limits = c(0,25), breaks = NULL) +
  #scale_x_continuous(limits = c(-.2, 1), breaks = c(0, .5, 1)) +
  theme_black()

get.ci(df.demo.filt$accuracy.chosen.chosen)
...
summary(lm(scale(accuracy.best.best) ~ scale(chosen.model.ll), data = df.demo.filt))
...
ggplot(df.demo.filt, aes(x = chosen.model.kappa, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm') +
  labs(x = '',
       y = '') +
...
summary(lm(scale(accuracy.best.best) ~ scale(chosen.model.kappa), data = df.demo.filt))
...
summary(glm(chose.correct.model ~ cc.correct.total, df.demo.filt, family = 'binomial'))
...
ggplot(df.demo.filt, aes(x = best.model.ll.magnitude, y = accuracy.best.best)) +
  geom_point(color='gray') +
  geom_smooth(method='lm',color='gray') +
  theme_black() +
  labs(x = '\nOOS likelihood of best model', y = 'Parameter accuracy')
ggplot(df.demo.filt, aes(x = best.model.ll.magnitude, y = chosen.model.ll)) +
  geom_point(color='gray') +
  geom_smooth(method='lm',color='gray') +
  theme_black() +
  labs(x = '\nOOS likelihood of best model', y = 'Process accuracy')


ggplot(df.demo.filt, aes(x = var.model.ll, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')
...
ggplot(df.demo.filt, aes(x = best.model.ll.magnitude, y = mse.best, color = inv.temp)) +
  geom_point() +
  geom_smooth(method='lm')
summary(lm(accuracy.best.best ~ best.model.ll.magnitude + inv.temp, data = df.demo.filt))
...
ggplot(df.demo.filt, aes(x = choice_domain, y = chosen.model.ll)) +
  ...
  scale_x_continuous(breaks = c(0,50,100)) +
  scale_y_continuous(breaks = c(0,.5,1))
summary(lm(scale(chosen.model.ll) ~ scale(choice_domain) +
             best.model.fac + best.model.ll.magnitude, data = df.demo.filt))

ggplot(df.demo.filt, aes(x = choice_exp, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')

ggplot(df.demo.filt, aes(x = confidence, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = confidence, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
...
ggplot(df.demo.filt, aes(x = decisionstyle, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = decisionstyle, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')
m.ds = lm(accuracy.best.best ~ decisionstyle, data = df.demo.filt)
summary(m.ds)

ggplot(df.demo.filt, aes(x = mindfulness, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm', color = 'brown') +
  labs(x = 'CAMS-R score', y = 'Parameter accuracy') +
  scale_x_continuous(breaks = c(2,10)) +
  scale_y_continuous(breaks = c(0, .5, 1))
ggplot(df.demo.filt, aes(x = mindfulness, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm', color = 'brown') +
  labs(x = 'CAMS-R score', y = 'Parameter accuracy') +
  scale_x_continuous(breaks = c(2,10)) +
...
summary(lm(accuracy.best.best ~ mindfulness + best.model.fac + best.model.ll.magnitude, data = df.demo.filt))
m.mindfulness = glm(chose.correct.model ~ mindfulness, data = df.demo, family = 'binomial')
summary(m.mindfulness)

m.mindfulness = glm(chosen.model.ll ~ mindfulness, data = df.demo.filt)
summary(m.mindfulness)
m.mindfulness = lm(accuracy.best.best ~ mindfulness, data = df.demo.filt)
summary(m.mindfulness)
cor.test(df.demo.filt$accuracy.best.best, df.demo.filt$mindfulness)
...
ggplot(test, aes(x = meditation_exp1, y = chose.correct.model.m)) +
  ...
  theme_black()
ggplot(df.demo.filt, aes(x = meditation_exp1, y = chosen.model.ll)) +
  geom_violin() +
  geom_point(data = test, aes(y = chosen.model.ll.m), size = 5, color = 'black') +
  geom_errorbar(data = test,
                aes(y = chosen.model.ll.m,
                    ymin = chosen.model.ll.m - chosen.model.ll.se,
                    ymax = chosen.model.ll.m + chosen.model.ll.se),
                width = .2, color = 'black') +
  #scale_y_continuous(limits = c(0.1, 0.4), breaks = c(0.1, 0.2, 0.3, 0.4), labels = c('10', '20', '30', '40')) +
  labs(x = '\nPrior meditation\nexperience', y = 'OOS likelihood of chosen model') +
  theme_black()
ggplot(df.demo.filt, aes(x = meditation_exp1, y = chose.correct.model)) +
  geom_violin() +
  geom_point(data = test, aes(y = chose.correct.model.m), size = 5, color = 'black') +
  geom_errorbar(data = test,
                aes(y = chose.correct.model.m,
                    ymin = chose.correct.model.m - chose.correct.model.se,
                    ymax = chose.correct.model.m + chose.correct.model.se),
                width = .2, color = 'black') +
  #scale_y_continuous(limits = c(0.1, 0.4), breaks = c(0.1, 0.2, 0.3, 0.4), labels = c('10', '20', '30', '40')) +
  labs(x = '\nPrior meditation\nexperience', y = '% reporting correct model') +
  theme_black()
ggplot(test, aes(x = meditation_exp1, y = accuracy.best.best.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = accuracy.best.best.m - accuracy.best.best.se, ymax = accuracy.best.best.m + accuracy.best.best.se))
summary(glm(chose.correct.model ~ meditation_exp1 + best.model.fac + best.model.ll.magnitude + icar_num_correct, data = df.demo.filt))
summary(lm(chosen.model.ll ~ meditation_exp1 + best.model.fac + best.model.ll.magnitude, data = df.demo.filt))
t.test(df.demo.filt$chosen.model.ll[df.demo.filt$meditation_exp1 == 'Yes'],
       df.demo.filt$chosen.model.ll[df.demo.filt$meditation_exp1 == 'No'])
power.t.test(delta = 0.1, sd = .35, power = .8)

summary(glm(meditation_exp1_bin ~ best.model.fac,
            df.demo.filt %>% mutate(meditation_exp1_bin = meditation_exp1 == 'Yes'),
            family = 'binomial'))

test2 = df.demo.filt %>% filter(meditation_exp1 == 'Yes') %>%
  ...
  summarize(accuracy.best.best.m = mean(accuracy.best.best, na.rm = T),
            ...
            chose.correct.model.m = mean(chose.correct.model, na.rm = T),
            chose.correct.model.se = se.prop(chose.correct.model))
ggplot(test2, aes(x = meditation_exp2, y = chosen.model.ll.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = chosen.model.ll.m - chosen.model.ll.se, ymax = chosen.model.ll.m + chosen.model.ll.se))
ggplot(test2, aes(x = meditation_exp2, y = chose.correct.model.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = chose.correct.model.m - chose.correct.model.se, ymax = chose.correct.model.m + chose.correct.model.se))

df.demo.filt = df.demo.filt %>% mutate(meditation_exp3.num = as.numeric(meditation_exp3),
                                       meditation_exp4.num = as.numeric(meditation_exp4))

ggplot(df.demo.filt, aes(x = meditation_exp3.num, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = meditation_exp3.num, y = accuracy.chosen.chosen)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = meditation_exp4.num, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = meditation_exp4.num, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')

test3 = df.demo.filt %>% filter(meditation_exp1 == 'Yes') %>%
  group_by(meditation_exp5) %>%
  summarize(accuracy.best.best.m = mean(accuracy.best.best, na.rm = T),
            accuracy.best.best.se = se(accuracy.best.best),
            accuracy.chosen.chosen.m = mean(accuracy.chosen.chosen, na.rm = T),
            accuracy.chosen.chosen.se = se(accuracy.chosen.chosen),
            ...
            chose.correct.model.m = mean(chose.correct.model, na.rm = T),
            chose.correct.model.se = se.prop(chose.correct.model))
ggplot(test3, aes(x = meditation_exp5, y = chosen.model.ll.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = chosen.model.ll.m - chosen.model.ll.se, ymax = chosen.model.ll.m + chosen.model.ll.se))
ggplot(test3, aes(x = meditation_exp5, y = chose.correct.model.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = chose.correct.model.m - chose.correct.model.se, ymax = chose.correct.model.m + chose.correct.model.se))

test4 = df.demo.filt %>% filter(meditation_exp1 == 'Yes') %>%
  group_by(meditation_exp6) %>%
  summarize(accuracy.best.best.m = mean(accuracy.best.best, na.rm = T),
            accuracy.best.best.se = se(accuracy.best.best),
            accuracy.chosen.chosen.m = mean(accuracy.chosen.chosen, na.rm = T),
            accuracy.chosen.chosen.se = se(accuracy.chosen.chosen),
            ...
            chose.correct.model.m = mean(chose.correct.model, na.rm = T),
            chose.correct.model.se = se.prop(chose.correct.model))
ggplot(test4, aes(x = meditation_exp6, y = chosen.model.ll.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = chosen.model.ll.m - chosen.model.ll.se, ymax = chosen.model.ll.m + chosen.model.ll.se))
ggplot(test4, aes(x = meditation_exp6, y = chose.correct.model.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = chose.correct.model.m - chose.correct.model.se, ymax = chose.correct.model.m + chose.correct.model.se))
...
ggplot(test, aes(x = chose.correct.model, y = pct.features.norm.m)) +
  ...
  theme_black()
ggplot(test, aes(x = chose.correct.model, y = pct.features.norm.reported.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = pct.features.norm.reported.m - pct.features.norm.reported.se,
                    ymax = pct.features.norm.reported.m + pct.features.norm.reported.se))

summary(lm(pct.features.norm ~ chose.correct.model + best.model.fac + best.model.ll.magnitude, data = df.demo.filt))
...
test = df.demo.filt %>% group_by(oneatt.correct) %>%
  summarize(oneatt.norm.actual.m = mean(oneatt.norm.actual),
            oneatt.norm.actual.se = se.prop(oneatt.norm.actual))
ggplot(test, aes(x = oneatt.correct, y = oneatt.norm.actual.m)) +
  geom_point(size = 5, color = 'white') +
  geom_errorbar(aes(ymin = oneatt.norm.actual.m - oneatt.norm.actual.se,
                    ymax = oneatt.norm.actual.m + oneatt.norm.actual.se),
                width = .2, color = 'white') +
  scale_x_discrete(labels = c('No', 'Yes')) +
  labs(x = 'Reported correct\nmodel', y = '% who actually did') +
  theme_black()

test = df.demo.filt %>% group_by(oneatt.correct, norm.oneatt) %>%
  summarize(oneatt.norm.actual.m = mean(oneatt.norm.actual),
            oneatt.norm.actual.se = se.prop(oneatt.norm.actual))
ggplot(test, aes(x = oneatt.correct, y = oneatt.norm.actual.m, color = norm.oneatt, group = norm.oneatt)) +
  geom_point(size = 5) +
  geom_errorbar(aes(ymin = oneatt.norm.actual.m - oneatt.norm.actual.se,
                    ymax = oneatt.norm.actual.m + oneatt.norm.actual.se),
                width = .2, color = 'white') +
...
summary(lm(scale(accuracy.best.best) ~ scale(icar_num_correct) + best.model.fac + best.model.ll.magnitude, df.demo.filt))
summary(lm(scale(chosen.model.ll) ~ scale(icar_num_correct) + best.model.fac + best.model.ll.magnitude, df.demo.filt))

summary(glm(chose.correct.model ~ scale(icar_num_correct) + best.model.fac + best.model.ll.magnitude, df.demo.filt, family = 'binomial'))

ggplot(df.demo.filt, aes(x = icar_num_correct, y = chosen.model.ll)) +
  geom_point(color = 'gray') +
  geom_smooth(method='lm', color = 'white') +
  theme_black() +
...
ggplot(df.demo.filt %>% mutate(iq.split = cut(icar_num_correct,2)), aes(x = iq.split, y = chose.correct.model)) +
  geom_point(color = 'gray') +
  geom_smooth(method='lm', color = 'white') +
  theme_black() +
  labs(x = 'IQ score', y = 'OOS likelihood of reported model')
summary(glm(chose.correct.model ~ icar_num_correct + best.model.fac + best.model.ll.magnitude, df.demo.filt, family = 'binomial'))

ggplot(df.demo.filt, aes(x = education, y = accuracy.best.best)) +
  geom_point(color = 'gray') +
  geom_smooth(method='lm', color = 'white') +
  theme_black() +
  labs(x = 'IQ score', y = 'Parameter accuracy')

ggplot(df.demo.filt, aes(x = maia, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = maia, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
summary(lm(accuracy.best.best ~ maia + chosen.model.fac + avg.model.ll, df.demo.filt))
summary(lm(chosen.model.ll ~ maia + chosen.model.fac + avg.model.ll, df.demo.filt))

ggplot(df.demo.filt, aes(x = sris.insight, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = sris.insight, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
summary(lm(accuracy.best.best ~ sris.insight + best.model.fac + avg.model.ll, df.demo.filt))
summary(lm(chosen.model.ll ~ sris.insight + best.model.fac + avg.model.ll, df.demo.filt))

ggplot(df.demo.filt, aes(x = sris.tendency, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')

ggplot(df.demo.filt, aes(x = acs.focusing, y = accuracy.best.best)) +
  geom_point(color = 'gray') +
  geom_smooth(method='lm', color = 'gray') +
...
m.acs1 = lm(accuracy.best.best ~ acs.focusing + acs.shifting +
...
ggplot(df.demo.filt, aes(x = orienting, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = orienting, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
...
ggplot(df.demo.filt, aes(x = accuracy.best.best, y = satisfaction)) +
  geom_point() +
  geom_smooth(method='lm')
df.demo.filt %>% group_by(pct.) %>% summarize(s = mean(satisfaction))
...
# stats
m.mods = lm(accuracy.best.best ~ decisionstyle + mindfulness + sris.tendency + sris.insight + maia + acs.shifting + acs.focusing +
              icar_num_correct + gender + age + edu.num + meditation_exp1 +
              alerting + orienting + exec +
              choice_domain + confidence + consistency1 +
              best.model.fac + best.model.ll.magnitude,
            data = df.demo.filt)
summary(m.mods)
...
features.graph = df.demo.filt %>% select(subject,oneatt.correct, binwts.correct, binatts.correct, chose.correct.model) %>%
  ...
  summarize(val = mean(value,na.rm=T),
            val.se = se(value)) %>%
  mutate(name = factor(name, c('oneatt.correct', 'binwts.correct', 'binatts.correct', 'chose.correct.model')))
features.graph.obs = df.demo.filt.obs %>% select(subject,oneatt.correct, binwts.correct, binatts.correct, chose.correct.model) %>%
  pivot_longer(!subject) %>%
  group_by(name) %>%
  summarize(val = mean(value,na.rm=T),
            val.se = se(value)) %>%
  mutate(name = factor(name, c('oneatt.correct', 'binwts.correct', 'binatts.correct', 'chose.correct.model')))
features.graph$type = 'Original'
features.graph.obs$type = 'Observers'
features.graph.both = full_join(features.graph, features.graph.obs)
features.graph.both = features.graph.both %>% mutate(type = factor(type, c('Original', 'Observers'), c('Original', 'Observers')))

ggplot(features.graph.both, aes(x = name, y = val, group = type, fill = type)) +
  geom_col(color='white', position = 'dodge') +
  geom_errorbar(aes(ymin = val - val.se, ymax = val + val.se), color = 'white', position = position_dodge(width = .9), width = .2) +
  geom_segment(x = 0, y = .5, xend = 3.5, yend = 0.5, color = 'red', linetype = 'dashed') +
  geom_segment(x = 3.5, y = 1/6, xend = 4.5, yend = 1/6, color = 'red', linetype = 'dashed') +
  theme_black() +
  labs(x = '', y = '% correct') +
  scale_x_discrete(labels = c('Q1\n(one vs.\nmultiple attributes)', 'Q2\n(binary vs.\ngraded weights)', 'Q3\n(binary vs.\ngraded attributes)', 'Overall model')) +
  theme(legend.title = element_blank()) +
...
prop.test(c(sum(df.demo.filt$oneatt.correct), sum(df.demo.filt.obs$oneatt.correct)),
          c(length(!is.na(df.demo.filt$oneatt.correct)), length(!is.na(df.demo.filt.obs$oneatt.correct))))
prop.test(c(sum(df.demo.filt$binwts.correct, na.rm = T), sum(df.demo.filt.obs$binwts.correct, na.rm = T)),
          c(length(!is.na(df.demo.filt$binwts.correct)), length(!is.na(df.demo.filt.obs$binwts.correct))))
prop.test(c(sum(df.demo.filt$binatts.correct, na.rm = T), sum(df.demo.filt.obs$binatts.correct, na.rm = T)),
          c(length(!is.na(df.demo.filt$binatts.correct)), length(!is.na(df.demo.filt.obs$binatts.correct))))
...
# likelihood of chosen model
mean.orig = mean(df.demo.filt.both$chosen.model.ll[df.demo.filt.both$type == 'Original'])
se.orig = se(df.demo.filt.both$chosen.model.ll[df.demo.filt.both$type == 'Original'])
mean.obs = mean(df.demo.filt.both$chosen.model.ll[df.demo.filt.both$type == 'Observers'])
se.obs = se(df.demo.filt.both$chosen.model.ll[df.demo.filt.both$type == 'Observers'])
...
## weight accuracy
mean.orig = mean(df.demo.filt.both$accuracy.best.best[df.demo.filt.both$type == 'Original'], na.rm = T)
se.orig = se(df.demo.filt.both$accuracy.best.best[df.demo.filt.both$type == 'Original'])
mean.obs = mean(df.demo.filt.both$accuracy.best.best[df.demo.filt.both$type == 'Observers'], na.rm = T)
se.obs = se(df.demo.filt.both$accuracy.best.best[df.demo.filt.both$type == 'Observers'])
ggplot(df.demo.filt.both, aes(x = accuracy.best.best, fill = type, group = type)) +
  geom_histogram(color = 'white', alpha = .7, bins = 25, position = position_identity()) +
  geom_vline(xintercept = mean.orig, linetype = 1, color = '#e3211c') +
  geom_vline(xintercept = mean.orig - se.orig, linetype = 'dashed', color = '#e3211c') +
  geom_vline(xintercept = mean.orig + se.orig, linetype = 'dashed', color = '#e3211c') +
...
## compare
df.demo.correct.obs = df.demo.filt.obs %>% group_by(chose.correct.model) %>%
  summarize(accuracy.m = mean(accuracy.best.best, na.rm = T), accuracy.se = se(accuracy.best.best))
ggplot(df.demo.filt.obs, aes(x = chose.correct.model, y = accuracy.best.best)) +
  geom_violin() +
  geom_point(data = df.demo.correct.obs, aes(y = accuracy.m)) +
  geom_errorbar(data = df.demo.correct.obs,
                aes(y = accuracy.m, ymin = accuracy.m - accuracy.se,
                    ymax = accuracy.m + accuracy.se),
                width = .1) +
  theme_black() +
...
summary(lm(scale(chosen.model.ll) ~ scale(icar_num_correct) + best.model.fac + scale(best.model.ll.magnitude), df.demo.filt.obs))
...
# meditation
meditation.graph = df.demo.filt.both %>% group_by(type, meditation_exp1) %>%
  summarize(accuracy.best.best.m = mean(accuracy.best.best, na.rm = T),
            accuracy.best.best.se = se(accuracy.best.best),
            chosen.model.ll.m = mean(chosen.model.ll, na.rm = T),
            chosen.model.ll.se = se(chosen.model.ll),
            chose.correct.model.m = mean(chose.correct.model, na.rm = T),
            chose.correct.model.se = se.prop(chose.correct.model))
ggplot(meditation.graph, aes(x = meditation_exp1, y = chosen.model.ll.m, group = type, color = type)) +
  geom_point(size = 5) +
  geom_line() +
  geom_errorbar(aes(ymin = chosen.model.ll.m - chosen.model.ll.se,
                    ymax = chosen.model.ll.m + chosen.model.ll.se),
                width = .1, color = 'white') +
  #scale_y_continuous(limits = c(0.1, 0.4), breaks = c(0.1, 0.2, 0.3, 0.4), labels = c('10', '20', '30', '40')) +
  labs(x = '\nPrior meditation experience', y = 'OOS likelihood of reported model') +
  theme_black() +
  theme(legend.title = element_blank()) +
  scale_color_brewer(palette = 'Set1')
ggplot(meditation.graph, aes(x = meditation_exp1, y = chose.correct.model.m, group = type, color = type)) +
  geom_point(size = 5) +
  geom_line() +
  geom_errorbar(aes(ymin = chose.correct.model.m - chose.correct.model.se,
                    ymax = chose.correct.model.m + chose.correct.model.se),
...
df.demo.filt.both = df.demo.filt.both %>% mutate(type.orig = type == 'Original',
                                                 meditation_exp1_yes = meditation_exp1 == 'Yes')
meditation.analysis1 = glm(chose.correct.model ~ meditation_exp1_yes * type.orig + best.model.fac +
                             best.model.ll.magnitude + icar_num_correct,
                           data = df.demo.filt.both)
summary(meditation.analysis1)
summary(lm(chosen.model.ll ~ meditation_exp1_yes * type.orig + best.model.fac + best.model.ll.magnitude + icar_num_correct,
           data = df.demo.filt.both))

# do matching shit
test = df.demo.filt.both %>% filter(!is.na(target_id_num)) %>% count(target_id_num)

df.demo.filt$oneatt.correct.obs = NA
for (i in 1:nrow(df.demo.filt)) {
  ...
  if (nrow(df.obs.cur) > 0) {
    df.demo.filt$oneatt.correct.obs[i] = mean(df.obs.cur$oneatt.correct)
  }
}

mean(df.demo.filt$oneatt.correct)
mean(df.demo.filt$oneatt.correct.obs, na.rm = T)
wilcox.test(as.numeric(df.demo.filt$oneatt.correct), df.demo.filt$oneatt.correct.obs, paired = T)
...
# get data for observers --------------------------------------------------

# practice trials
df.s1.prac.obs = df.s1.practice.filt %>%
  mutate(subject_num = subject.num) %>%
  select(subject, subject_num, version, attributes, opt1_values, opt2_values, choice)
subjlist.cur = unique(df.s1.prac.obs$subject)
prac.json = character(length(subjlist.cur))
for (subj.ind in 1:length(subjlist.cur)) {
  prac.json[subj.ind] = toJSON(df.s1.prac.obs %>% filter(subject == subjlist.cur[subj.ind]))
}

prac.json.towrite = paste0("prac_trials_db = [", paste(prac.json, collapse = ","), "];")
write(prac.json.towrite, 'practice_trials_db.js')

# real trials
df.s1.obs = df.s1.filt %>%
  mutate(subject_num = subject.num) %>%
  select(subject, subject_num, version, attributes, opt1_values, opt2_values, choice)
...
# for observer analysis
write.table(df.demo %>% select(subject, subject.num), 'observer_mapping.csv', row.names = F, col.names = F, sep = ",")
write.table(atts, 'att_order.csv', row.names = F, col.names = F, sep = ",")
