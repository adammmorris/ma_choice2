...
atts_lb = readmatrix(strcat(datapath, 'modeling_lb.csv'));
atts_ub = readmatrix(strcat(datapath, 'modeling_ub.csv'));
...
for i = 1:length(data_real_scaled)
    data_real_scaled(i).options = signOptions(data_real_scaled(i).options,false,true,atts_lb,atts_ub);
end
