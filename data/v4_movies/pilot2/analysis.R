...
for (i in 1:nrow(df.attributes)) {
  ...
  cur.scale = c('Nobody', 'A few people', 'Half of people', 'Most people', 'Everyone')
...
ggplot(df.demo.filt, aes(x = icar_num_correct, y = accuracy.chosen.chosen)) +
  geom_point() +
  geom_smooth(method='lm')
...
# stats
m.mods = lm(accuracy.chosen.chosen ~ decisionstyle + mindfulness + sris.tendency + sris.insight + maia + acs.shifting + acs.focusing +
