...
for (i in 1:nrow(df.demo)) {
  ...
  df.attributes$best.model.num[as.character(df.attributes$subject) == df.demo$subject[i]] = df.demo$best.model.num[i]
  ...
  df.demo$norm.model.ll[i] = df.cv.norm[i,df.demo$norm.model.num[i]]
  df.demo$norm.model.ll.unnormed[i] = df.cv[i,df.demo$norm.model.num[i]]
}
df.demo = df.demo %>%
...
for (i in 1:nrow(df.demo)) {
  ...
  df.attributes$best.model.num.rnd[as.character(df.attributes$subject) == df.demo$subject[i]] = df.demo$best.model.num.rnd[i]
...
df.attributes.subj = df.attributes %>% group_by(subject) %>%
  summarize(num_same = sum(same & (rating.wad > .1)),
            accuracy.wad.wad = cor(fitted.weight.wad, rating.wad.signed),
...
for (i in 1:nrow(df.demo)) {
  att.row = as.character(df.attributes.subj$subject) == df.demo$subject[i]
...
df.demo = df.demo %>% mutate(
  ...
  meditation_minutes = meditation_exp3 * meditation_exp4,
...
for (i in 1:nrow(df.demo)) {
  ...
  if (subj %in% demo_rand$subject) {
    df.demo$in_rand[i] = T
    demo_stamp = df.demo$stamp[i]
    rand_stamp = demo_rand$stamp[demo_rand$subject == subj]
    if (demo_stamp < rand_stamp) {
      df.demo$which_version_first[i] = 'movies'
    } else {
      df.demo$which_version_first[i] = 'rand'
    }
  } else {
    df.demo$in_rand[i] = F
  }
...
for (subj in subjlist) {
  ...
  if (df.demo$instruction_times_median[demo.row] < 2 ||
      ...
      (df.demo$in_rand[demo.row] & df.demo$which_version_first[demo.row] == 'rand') ||
      #df.attributes.subj$num_same[df.attributes.subj$subject == subj] > 0 |
      df.demo$sd.model.ll[demo.row] < 0.01 | 
...
test = df.demo.filt %>% select(subject,oneatt.correct, binwts.correct, binatts.correct) %>%
  pivot_longer(!subject) %>%
  group_by(name) %>%
  summarize(val = mean(value,na.rm=T),
            val.se = se(value)) %>%
  mutate(name = factor(name, c('oneatt.correct', 'binwts.correct', 'binatts.correct')))
ggplot(test, aes(x = name, y = val)) +
  geom_col(color='white') +
  geom_errorbar(aes(ymin = val - val.se, ymax = val + val.se), width = .2, color = 'white') +
  geom_hline(yintercept = .5, color = 'red', linetype = 'dashed') +
  theme_black() +
  labs(x = '', y = '% correct') +
  scale_x_discrete(labels = c('Q1\n(one vs. multiple attributes)', 'Q2\n(binary vs. graded weights)', 'Q3\n(binary vs. graded attributes)'))
...
ggplot(df.demo.filt, aes(x = chosen.model.ll)) +
  geom_histogram(color = 'black', fill = 'gray', bins = 25) +
  labs(x = "Strategy awareness",
...
for (i in 1:length(rand.errs)) {
  ...
  for (j in 1:nrow(df.demo.filt)) {
    subj.rows = as.character(df.attributes$subject) == as.character(df.demo.filt$subject[j])
...
ggplot(df.demo.filt, aes(x = accuracy.best.best)) +
  geom_histogram(alpha = .3, fill = 'black', color = 'black') +
  #geom_vline(xintercept = mean(rand.errs), linetype = 'dashed', color = 'gray') +
  #geom_vline(xintercept = mean(df.demo.filt$accuracy.best.best, na.rm = T), linetype = 1, color = 'black') +
  labs(x = 'Weight awareness', y = 'Number of subjects') +
  scale_y_continuous(limits = c(0,25), breaks = NULL) +
  scale_x_continuous(limits = c(-.2, 1), breaks = c(0, .5, 1))
  #theme_black()
get.ci(df.demo.filt$accuracy.wad.wad)
...
ggplot(df.demo.filt, aes(x = chose.correct.model, y = accuracy.best.best)) +
  ...
  geom_errorbar(data = df.demo.correct,
                ...
                width = .1)
summary(lm(accuracy.best.best ~ chose.correct.model, data = df.demo.filt))
...
ggplot(df.demo.filt, aes(x = mindfulness, y = accuracy.wad.wad)) +
  geom_point() +
...
m.mindfulness = lm(chosen.model.ll ~ mindfulness, data = df.demo.filt)
...
test = df.demo.filt %>% group_by(meditation_exp1) %>%
  filter(meditation_exp1 != 'Less than 1 year') %>%
  summarize(accuracy.best.best.m = mean(accuracy.best.best, na.rm = T),
            accuracy.best.best.se = se(accuracy.best.best),
            accuracy.wad.wad.m = mean(accuracy.wad.wad, na.rm = T),
            accuracy.wad.wad.se = se(accuracy.wad.wad),
            chosen.model.ll.m = mean(chosen.model.ll, na.rm = T),
            chosen.model.ll.se = se(chosen.model.ll),
            chosen.model.dist.m = mean(chosen.model.dist, na.rm = T),
            chosen.model.dist.se = se(chosen.model.dist),
            chosen.model.rnd = mean(chosen.model.rnd, na.rm = T),
            chosen.model.rnd.se = se(chosen.model.rnd),
            chose.correct.model.m = mean(chose.correct.model, na.rm = T),
            chose.correct.model.se = se.prop(chose.correct.model),
            pct.features.correct.m = mean(pct.features.correct),
            pct.features.correct.se = se(pct.features.correct),
            pct.features.norm.m = mean(pct.features.norm),
            pct.features.norm.se = se(pct.features.norm),
            best.model.ll.magnitude.m = mean(best.model.ll.magnitude))
ggplot(test, aes(x = meditation_exp1, y = chosen.model.ll.m)) +
  ...
  geom_errorbar(aes(ymin = chosen.model.ll.m - chosen.model.ll.se,
                    ...
                    width = .2)) +
  scale_y_continuous(limits = c(0.6, 0.8), breaks = c(0.6, 0.8), labels = c('60%', '80%')) +
  labs(x = 'Prior meditation\nexperience?', y = 'Likelihood of\nreported model')
ggplot(test, aes(x = meditation_exp1, y = chose.correct.model.m)) +
  ...
  labs(x = 'Prior meditation\nexperience', y = '% reporting the\ncorrect model')
ggplot(test, aes(x = meditation_exp1, y = accuracy.wad.wad.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = accuracy.best.best.m - accuracy.best.best.se, ymax = accuracy.best.best.m + accuracy.best.best.se))
ggplot(test, aes(x = meditation_exp1, y = pct.features.correct.m)) +
  geom_point() +
  geom_errorbar(aes(ymin = pct.features.correct.m - pct.features.correct.se,
                    ymax = pct.features.correct.m + pct.features.correct.se))
summary(glm(chose.correct.model ~ meditation_exp1, data = df.demo.filt))
summary(lm(chosen.model.ll ~ meditation_exp1 + icar_num_correct, data = df.demo.filt %>% filter(meditation_exp2 != 'Less than 1 year')))
t.test(df.demo.filt$chosen.model.ll[df.demo.filt$meditation_exp1 == 'Yes' & df.demo.filt$meditation_exp2 != 'Less than 1 year'],
       df.demo.filt$chosen.model.ll[df.demo.filt$meditation_exp1 == 'No'])
power.t.test(delta = 0.12, sd = .3, power = .8)
get.ci(df.demo.filt$chosen.model.ll[df.demo.filt$meditation_exp1 == 'Yes'])
get.ci(df.demo.filt$chosen.model.ll[df.demo.filt$meditation_exp1 == 'No'])

ggplot(test, aes(x = meditation_exp1, y = pct.features.norm.m)) +
  geom_point(size = 5) +
...
ggplot(df.demo.filt, aes(x = meditation_minutes, y = chosen.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = meditation_exp3.num, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')
...
ggplot(df.demo.filt, aes(x = meditation_exp4.num, y = accuracy.best.best)) +
  geom_point() +
  geom_smooth(method='lm')
summary(lm(chosen.model.dist ~ meditation_minutes, df.demo.filt))
...
ggplot(df.demo.filt, aes(x = norm.model.ll)) +
  geom_histogram(color = 'black', fill = 'gray', bins = 25) +
  labs(x = "Normative strategy\nalignment",
       y = "Number of subjects") +
  scale_x_continuous(breaks = c(0,.5,1)) +
  scale_y_continuous(breaks = NULL) #+
#theme_black() +
...
summary(lm(chosen.model.ll ~ best.model.ll.magnitude + best.model.fac, df.demo.filt))
summary(lm(norm.model.ll ~ best.model.ll.magnitude + best.model.fac, df.demo.filt))

ggplot(df.demo.filt, aes(x = chosen.model.ll, y = pct.features.norm)) +
  geom_point() +
  geom_smooth(method='lm')
ggplot(df.demo.filt, aes(x = chosen.model.ll, y = norm.model.ll)) +
  geom_point() +
  geom_smooth(method='lm')
test = df.demo.filt %>% group_by(chose.correct.model) %>%
  summarize(pct.features.norm.m = mean(pct.features.norm==1),
            ...
            norm.model.ll.m = mean(norm.model.ll),
            norm.model.ll.se = se(norm.model.ll))
...
ggplot(test, aes(x = chose.correct.model, y = pct.features.norm.m)) +
  ...
  geom_errorbar(aes(ymin = pct.features.norm.m - pct.features.norm.se,
                    ...
                    width = .2)) +
  scale_x_discrete(labels = c('No', 'Yes')) +
  labs(x = 'Reported correct\nstrategy?', y = 'Normative strategy alignment')
...
ggplot(test, aes(x = chose.correct.model, y = norm.model.ll.m)) +
  geom_point(size = 5) +
  geom_errorbar(aes(ymin = norm.model.ll.m - norm.model.ll.se,
                    ymax = norm.model.ll.m + norm.model.ll.se),
                width = .2) +
  scale_x_discrete(labels = c('No', 'Yes')) +
  scale_y_continuous(breaks = c(0.6, 0.8)) +
  labs(x = 'Reported correct\nstrategy?', y = 'Average normative\nstrategy alignment')

summary(lm(pct.features.norm ~ chose.correct.model*meditation_exp1 + chosen.model.fac + avg.model.ll, data = df.demo.filt))
...
ggplot(df.demo.filt, aes(x = pct.features.correct, y = satisfaction)) +
  geom_point() +
  geom_smooth(method='lm')
...
# stats
m.mods = lm(accuracy.wad.wad ~ decisionstyle + mindfulness + sris.tendency + sris.insight + maia + acs.shifting + acs.focusing +
